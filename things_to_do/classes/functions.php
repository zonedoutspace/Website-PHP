	...
	class Article {
		...
		*/
		...
		public static function getById( $id ) {
			...
			$sql = "select *, UNIX_TIMESTAMP(publicationDate) as publicationDate from do_things where id = :id";
			$st = $conn->prepare( $sql );
		...
		public static function getList( $numRows = 1000000, $order = "publicationDate desc" ) {
			...
			$conn = new PDO( DB_DSN, DB_USERNAME, DB_PASSWORD );
			$sql = "select SQL_CALC_FOUND_ROWS *, UNIX_TIMESTAMP(publicationDate) as publicationDate from do_things
		...
		public function insert() {
			...
			$conn = new PDO( DB_DSN, DB_USERNAME, DB_PASSWORD );
			$sql = "insert into do_things (publicationDate, title, content, imageExtension ) values ( FROM_UNIXTIME(:publicationDate), :title, :content, :imageExtension )";
			$st = $conn->prepare( $sql );
		...
		public function update() {
		    ...
		    // Update the Article
		    $conn = new PDO( DB_DSN, DB_USERNAME, DB_PASSWORD );
		    $sql = "update do_things set publicationDate = FROM_UNIXTIME(:publicationDate), title = :title, content = :content, imageExtension = :imageExtension where id = :id";
		    $st = $conn->prepare ( $sql );
		    $st->bindValue( ":publicationDate", $this->publicationDate, PDO::PARAM_INT );
		 ...
		 public function delete() {
		    ...
		    // Delete the Article
		    $conn = new PDO( DB_DSN, DB_USERNAME, DB_PASSWORD );
		    $st = $conn->prepare ( "delete from do_things where id = :id limit 1" );
		    $st->bindValue( ":id", $this->id, PDO::PARAM_INT );
		    $st->execute();
		    $conn = null;
